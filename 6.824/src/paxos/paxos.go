...
func (px *Paxos) Propose(seq int, v interface{}) {
	...
	//fmt.Println("seq:", seq, " from:", px.me, " acceptor va:", px.instance[seq].acceptor.va)
	...
	for px.instance[seq].status == Pending {
			...
			if prepareReply.Status == Pending {
				...
				if prepareReply.Result == OK {
					...
					if prepareReply.Va != nil {
						...
						//fmt.Println("change proper value", " seq:", seq, " from:", px.me, " give value:", px.instance[seq].proposer.value)
		...
		if prepareOK > len(px.peers)/2 {
			...
			args.Kinds = Accept
			args.N = px.instance[seq].proposer.n
			if px.instance[seq].proposer.value == nil {
				args.Value = v
			} else {
			...
			for i := 0; i < len(px.peers); i++ {
				...
				if i != px.me {
					...
					//fmt.Println("Accept seq:", seq, " from:", px.me, " to:", i, "value:", args.Value, " reply.status:", acceptReply.Status)
				} else {
			...
			if acceptOK > len(px.peers)/2 {
				...
				for i := 0; i < len(px.peers); i++ {
					...
					if i != px.me {
						...
						//fmt.Println("Decide seq:", seq, " from:", px.me, " to:", i, " value:", args.Value, " reply.status:", decidedReply.Status)
					} else {
	...
	if px.instance[args.InsNum].status != Pending {
		...
		if args.Kinds == Prepare {
			...
			if args.N > px.instance[args.InsNum].acceptor.np {
				...
				//fmt.Println("Acceptor Prepare", " seq:", args.InsNum, " to:", px.me, " va:", reply.Va)
...
func (px *Paxos) Done(seq int) {
	...
	px.discardArgs[px.me] = px.currentDoneArg
...
func (px *Paxos) Min() int {
	...
	//must shrink the memory, including status is Forgotten, and other memory about value
	for i := 0; i <= minCompDone; i++ {
		if px.instance[i] != nil && px.instance[i].status == Decided {
			px.instance[i].status = Forgotten
			px.instance[i].final = nil
			px.instance[i].acceptor.va = nil
			px.instance[i].proposer.value = nil
		}
	}
...
func Make(peers []string, me int, rpcs *rpc.Server) *Paxos {
	...
	px.instance = make([]*Instance, 5000*100, 10000*100)
	px.currentMin = -1
	px.currentMax = -1
	px.currentDoneArg = -1
