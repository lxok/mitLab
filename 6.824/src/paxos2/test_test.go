...
func ndecided(t *testing.T, pxa []*Paxos, seq int) int {
	...
	for i := 0; i < len(pxa); i++ {
		if pxa[i] != nil {
			...
			//			if len(pxa) > 4 && pxa[4] != nil {
			//				decided4, v14 := pxa[4].Status(seq)
			//				decided3, v13 := pxa[3].Status(seq)

			//				fmt.Println("Test Peer:", i, " value:", v1, " Peer:4", " value:", v14, " status:", decided4, " Peer:3", v13, "status:", decided3)
			//			}
